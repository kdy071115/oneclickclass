...
import { attendanceService, detailService } from '../api/services';
...
export function QrPage() {
  ...
  const [label, setLabel] = useState('강의 출석');
  ...
  const loadQr = useCallback(async (refresh = false) => {
    ...
    setError('');
    try {
      const [session, detail, rows] = await Promise.all([
        refresh ? attendanceService.refreshQr(id) : attendanceService.issueQr(id),
        detailService.getClass(id),
        attendanceService.checkins(id),
      ]);
      ...
      setLabel(detail.title);
      setTotal(detail.enrolled);
      setChecked(rows.filter((row) => Boolean(row.checkedAt)).length);
    } catch {
