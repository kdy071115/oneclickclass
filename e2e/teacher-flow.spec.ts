...
test('강의자 상세 화면과 핵심 운영 페이지를 빠짐없이 이동한다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '어떻게 진행하나요?' })).toBeVisible();
  await page.goto(`${baseUrl}/classes/notion`);
...
test('강의자 운영 화면의 주요 액션이 실제 상태를 바꾼다', async ({ page }) => {
  ...
  await expect(page.locator('.operation-table .oc-table-row')).toHaveCount(3);
  ...
  await expect(page).toHaveURL(`${baseUrl}/applicants/1`);
